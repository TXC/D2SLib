...
namespace D2Shared.Huffman
{
        ...
        public static readonly IReadOnlyDictionary<char, string> TABLE = new Dictionary<char, string>(38)
    {
        ...
        public InternalBitArray EncodeChar(char source)
        {
            ...
            return encodedSymbol is not null ? encodedSymbol : throw new InvalidOperationException("Could not encode with an empty tree.");
        }
        ...
        public char DecodeChar(IBitReader reader)
        {
            ...
            while (!(current?.IsLeaf() ?? true))
            {
                ...
                if (bit == true && current.Right is not null)
                {
                    current = current.Right;
                }
                else if (bit == false && current.Left is not null)
                {
                    current = current.Left;
                }
            }
