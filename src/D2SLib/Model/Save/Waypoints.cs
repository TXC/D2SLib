...
public sealed class WaypointsSection : IDisposable
{
    ...
    public WaypointsDifficulty Normal => _difficulties[0];      // 0x0281
    public WaypointsDifficulty Nightmare => _difficulties[1];   // 0x0299
    public WaypointsDifficulty Hell => _difficulties[2];        // 0x02B1
...
public sealed class WaypointsDifficulty : IDisposable
{
    ...
    public void Write(IBitWriter writer)
    {
        ...
        writer.Align();
        Span<byte> padding = stackalloc byte[Padding];
        padding.Clear();
