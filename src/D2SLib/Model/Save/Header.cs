...
public class Header
{
    ...
    public static Header Read(IBitReader reader)
    {
        var header = new Header
        {
            ...
            Version = (SaveVersion)(reader.ReadUInt32() ?? 0xffffff),
            Filesize = reader.ReadUInt32() ?? 0,
            Checksum = reader.ReadUInt32() ?? 0
        };
        if (Core.Importer is VersionedImporter import)
        {
            import.SetVersion(header.Version);
        }
        return header;
    }
